...
public class BlinnPhongMaterial extends Material {
	...
	/**
	 * Instantiates a new blinn phong material.
	 *
	 * @param diffuse the diffuse
	 * @param specular the specular
	 * @param exponent the exponent
	 */
	public BlinnPhongMaterial(final MyColor diffuse, final MyColor specular,
			final int exponent) {
		if (diffuse == null) {
	...
	public MyColor colorFor(final Hit hit, final World world,final Tracer tracer) {
		...
		for (Light light : world.lightList) {
			...
			if (light.illuminates(hit.ray.at(hit.t),world)) {
				...
				final Vector3 h = lightVector.add(e).normalized();

				final double NdotH = h.dot(normal);

				final double maxNL = Math.max(0.0, lightVector.dot(normal));
				final double maxER = Math.pow(Math.max(0.0, NdotH), exponent);
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		BlinnPhongMaterial other = (BlinnPhongMaterial) obj;
		if (diffuse == null) {
